window.addEventListener('load', function () {
    ...
    let deck: karte[] = [];             // ungemischter Kartenstapel
    let spielerKarten: karte[] = [];    // Array der Spielerkarten
    let gegnerKarten: karte[] = [];     // Array der Gegnerkarten
    let ablageStapel: karte[] = [];     // Array der gespielten Karten

    // Deklarierung von Hilfsvariablen
    let spielKarte: karte;              // aktive Karte
    let spielerAktiv: boolean;          // Spieler aktiv?
    let bereitsGezogen: boolean;        // Karte bereits gezogen?
    ...
    function kartenAnzeigen(array: karte[]) {
        var klasse: string = "";
    ...
    function erstelleSichtbareKarte(array: karte[], arrayIndex: number, klasse: string) {
        var einfuegeDiv: HTMLElement = knotenErstellen(klasse, array[arrayIndex].farbe);
        
        var i = 5;
        while (i) {
            var nummerP: HTMLElement = document.createElement("p");
    ...
    function erstelleVerdeckteKarte(array: karte[], arrayIndex: number, klasse: string) {
        var einfuegeDiv: HTMLElement = knotenErstellen(klasse, "backside");
    
        var image: HTMLElement = document.createElement("div");
        image.setAttribute("class", ".verdeckt");
        einfuegeDiv.appendChild(image);
    }

    function htmlZurueckSetzen(klasse: string) {
        var htmlKnoten: HTMLElement = document.getElementById(klasse);
        while (htmlKnoten.firstChild) {
            htmlKnoten.removeChild(htmlKnoten.firstChild);
        }
    }

    function knotenErstellen(klasse: string, farbe: string): HTMLElement {
        var einfuegeDiv: HTMLElement = document.createElement("div");
    ...
    function kartenStapelGenerieren() {
        for (var i = 0; i < 4; i++) {
            ...
            if (i == 0) { farbe = "pink"; }
            else if (i == 1) { farbe = "rot"; }
            else if (i == 2) { farbe = "gelb"; }
            else if (i == 3) { farbe = "blau"; }
    ...
    function kartenGeben(anzahl: number) {
        while (anzahl) {
    ...
    function kartenMischen(array: karte[]) {
    ...
    function zaehleSpielbareKarten(): number {
    ...
    function karteLegen(zuSpielendeKarte: karte, index: number) {
        if (spielerAktiv) {
    ...
    function validiereKarte(karte1: karte, karte2: karte): boolean {
